...
// UserSignup: 通过用户名及密码完成user表(tbl_user)的注册操作
...
// UserSignin: 判断密码是否一致
func UserSignin(username string, enpcd string) bool {
	...
	// 以字符切片接受是不必要的
	if len(pRows) > 0 && string(pRows[0]["user_pwd"].([]byte)) == enpcd {
		return true
	}
	return false
}
...
func UpdateToken(username string, token string) bool {
	...
	if err != nil {
		fmt.Println("Failed to refresh token, err:" + err.Error())
		return false
	}
	return true
}

// IsTokenEqual: 判断token是否一致
func IsTokenEqual(username string, token string) bool {
